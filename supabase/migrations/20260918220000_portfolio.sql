...
--
-- Every migration in this folder is re-runnable: CI applies the whole folder on
-- each push, so a second run must be a no-op.
...
-- Seed content, skipped once a table holds anything — a re-run must never
-- duplicate rows or undo an edit made in the admin.
...
if not exists (select 1 from work_files) then
...
if not exists (select 1 from hobbies) then
...
if not exists (select 1 from movies) then
